...
from utils import coordinates, extract_mesh, colormap_image, tsdf_disc
...
##################################################################################
        # self.load_conv("output/scannet/scene0050_00/dim1_level18_128_0.1_intinit_0.5sphere_convnotrain_bestbest/checkpoint4651.pt")
...
##################################################################################
        ...
        with torch.no_grad():
            ...
            self.tsdf_volume = torch.zeros((tsdf_volume.shape)).to(self.device)
            self.tsdf_bounds = torch.tensor(bounds).to(self.device).float()
##################################################################################
    ...
    def create_bounds(self):
        ...
        '''
        self.bounding_box = torch.from_numpy(np.array(self.config['mapping']['bound'])).float().to(self.device)
    ...
    def load_conv(self, load_path):
        '''
        Load the model parameters and the estimated pose
        '''
        state_dict = torch.load(load_path)
        # Define the specific layer you want to update
        specific_layer_1 = 'quantize_b.quantize_conv_b.weight'  # Change this to your layer's name
        specific_layer_2 = 'quantize_b.quantize_conv_b.bias'
        new_state_dict = {specific_layer_1: state_dict['model'][specific_layer_1],specific_layer_2: state_dict['model'][specific_layer_2]}
    ...
    def get_loss_from_ret(self, ret, rgb=True, sdf=True, depth=True, fs=True, smooth=False, latent=True,):
        '''
        ...
        if latent:
            loss +=  self.config['training']['latent_weight'] * ret["diff"]
    ...
    def tracking_render(self, batch, frame_id):
        ...
        if self.config["fuse_tsdf"]:
            # tsdf fusion predict
            # print(f'\nfusion_track_idx: {frame_id}')
            tsdf_c2w = cur_c2w.clone().detach().cpu().numpy()
            tsdf_c2w[:3, 1] *= -1.0
            tsdf_c2w[:3, 2] *= -1.0
            ...
            self.tsdf_vol_pred.integrate(color, depth, self.intrinsic, tsdf_c2w, obs_weight=1.)
            
            tsdf_volume, _, _ = self.tsdf_vol_pred.get_volume()
            tsdf_volume = torch.tensor(tsdf_volume)
            tsdf_volume = tsdf_volume.reshape(1, 1, tsdf_volume.shape[0], tsdf_volume.shape[1], tsdf_volume.shape[2])
            tsdf_volume = tsdf_volume.permute(0, 1, 4, 3, 2).to(self.device)
            self.tsdf_volume[:] = tsdf_volume
        ...
        for i in range(self.config['tracking']['iter']):
            ...
            loss = self.get_loss_from_ret(ret,latent=False)
        ...
        if self.config['gt_cam']:
            ...
            best_c2w_est[0,:3] = c2w_gt[:3]

        if self.config["fuse_tsdf"]:
            # tsdf fusion predict
            # print(f'refusion_track_idx: {frame_id}')
            tsdf_c2w = self.est_c2w_data[frame_id].clone().detach().cpu().numpy()
            tsdf_c2w[:3, 1] *= -1.0
            tsdf_c2w[:3, 2] *= -1.0
    ...
    def create_optimizer(self):
        ...
        trainable_parameters = [{'params': self.model.decoder.parameters(), 'weight_decay': 1e-6, 'lr': self.config['mapping']['lr_decoder']},
                                ...
                                {'params': self.model.quantize_b.parameters(), 'weight_decay': 1e-6, 'lr': self.config['mapping']['lr_quant']}]
        ...
        if self.config['init_sphere']:
            sphere_parameters = [{'params': self.model.decoder.parameters(), 'weight_decay': 1e-6, 'lr': self.config['mapping']['lr_decoder']},
                                    {'params': self.model.embed_fn.parameters(), 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed']},
                                    {'params': self.model.quantize_b.parameters(), 'weight_decay': 1e-6, 'lr': self.config['mapping']['lr_quant']}]  
            self.sphere_optimizer = optim.Adam(sphere_parameters, betas=(0.9, 0.99))    
    ...
    def train_sphere(self, sample_points=256, voxel_size=0.1, margin=0.05, color=False):
        center = (self.bounding_box[:,1] + self.bounding_box[:,0])/2.
        radius = ((self.bounding_box[:,1] - self.bounding_box[:,0])/2.).min()
        volume = self.bounding_box[:, 1] - self.bounding_box[:, 0]
        ...
        for i in range(self.config['init_sphere']):
            self.map_optimizer.zero_grad()
            coords = coordinates(list(map(lambda x: x-100,self.tsdf_numpy.shape[2:]))[::-1], self.device).float().t() #70
            pts = (coords + torch.rand_like(coords)) * 4./256 + self.bounding_box[:, 0]
            pts_tcnn = (pts - self.bounding_box[:, 0]) / (self.bounding_box[:, 1] - self.bounding_box[:, 0])
            # Forward
            if self.config["fuse_tsdf"]:
                sdf,diff = self.model.query_sdf(pts_tcnn,tsdf_numpy=self.tsdf_volume, tsdf_bounds=self.tsdf_bounds) 
            else:
                sdf,diff = self.model.query_sdf(pts_tcnn,tsdf_numpy=self.tsdf_numpy, tsdf_bounds=self.tsdf_bounds)     
            target_sdf = radius - (center - pts).norm(dim=-1)
            if diff == None:
                loss = torch.nn.functional.mse_loss(sdf, target_sdf)
            else:
                loss = torch.nn.functional.mse_loss(sdf, target_sdf) +  0.5 * diff
    ...
    def run(self):
        ...
        for i, batch in tqdm(enumerate(data_loader)):
            ...
            # Tracking + Mapping
                ...
                if self.config['tracking']['iter_point'] > 0: #0
                    self.tracking_pc(batch, i)
                self.tracking_render(batch, i)
    
                if i%self.config['mapping']['map_every']==0: #5
                    self.current_frame_mapping(batch, i)
                    self.global_BA(batch, i)

                    
                # Add keyframe
                if i % self.config['mapping']['keyframe_every'] == 0: #5
                    self.keyframeDatabase.add_keyframe(batch, filter_depth=self.config['mapping']['filter_depth'])
